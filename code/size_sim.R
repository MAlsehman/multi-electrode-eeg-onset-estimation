# size_sim.R
# Trial-number sensitivity simulation for the EEG onset project.
# Runs 1,000 iterations at 25, 50, 75 and 100 trials per condition.
...
# Trial-size sensitivity: 1,000 iterations at each trial count
niter  <- 1000
sizes  <- c(25, 50, 75, 100)
notify <- 100

pe_list <- vector("list", length(sizes) * niter)
vi_list <- vector("list", length(sizes) * niter)
k <- 0

for(nt in sizes){
  Nt <- nt
  message("sample size ", Nt, " trials per condition")
  ...
  for(it in seq_len(niter)){
    ...
    pe_list[[k]] <- mutate(r$perelec, iter = it, Nt = Nt)
    vi_list[[k]] <- mutate(r$virtual, iter = it, Nt = Nt)
  }
}
...
save(
  perelec_all,
  virtual_all,
  file = "data/simres_1000_sizes.RData"
)
